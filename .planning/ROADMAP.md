...
- [ ] **Phase 2.1: NFC Fix** *(INSERTED)* - Physical NTAG215 student cards are accepted at the canteen cashier
...
### Phase 2.1: NFC Fix *(INSERTED)*
**Goal**: Physical NTAG215 student cards are successfully read by both Arduino variants (R3 and R4) and payments complete end-to-end at the canteen cashier
**Depends on**: Nothing (urgent hardware fix, unblocks physical card payments)
**Requirements**: NFC-FIX-01, NFC-FIX-02, NFC-FIX-03, NFC-FIX-04
**Success Criteria** (what must be TRUE):
  1. R4 Arduino (UNO R4 WiFi) reads NTAG215 UID without stalling after a failed read
  2. R3 Arduino emits `CARD|<14-hex-uid>` on serial when an NTAG215 is tapped
  3. `arduino_bridge.py` receives `CARD|` and `NFC|` serial lines and posts payments to `/api/pay`
  4. Cashier route accepts 14-character NTAG215 UIDs (not only 8-character MIFARE UIDs)
**Plans**: 4 plans
Plans:
- [ ] 32.1-01-PLAN.md — R4 firmware: fix RF field toggle + UID-length dispatch
- [ ] 32.1-02-PLAN.md — R3 firmware: add RF toggle + NTAG215 detection + serial emit
- [ ] 32.1-03-PLAN.md — ArduinoBridge rewrite: background listener + NFC|/CARD| parsing
- [ ] 32.1-04-PLAN.md — cashier_routes UID regex fix + integration tests
...
Note: Phase 2.1 is an urgent insertion — it can run in parallel with Phase 2 or independently. Phases 2, 3, and 5 all depend on Phase 1 and can be started independently once Phase 1 is complete.
...
| 2.1. NFC Fix *(INSERTED)* | 0/4 | Not started | - |
