...
class ConfigValidator:
    ...
    def _validate_environment_variables(self):
        ...
        for var, description in required_vars.items():
            ...
            if value:
                ...
                result = ValidationResult(
                    False, f"{var}: MISSING - {description}", {"required": True}
                )
        ...
        for var, description in optional_vars.items():
            ...
            if value:
                self.results.append(
                    ValidationResult(True, f"{var}: {description}", {"value": value})
                )
            else:
                self.results.append(
                    ValidationResult(
        ...
        try:
            ...
            for sheet_name, expected_columns in required_schema.items():
                try:
                    ...
                    result = ValidationResult(
                        False, f"Sheet '{sheet_name}' NOT FOUND", {"required": True}
                    )
                    self.results.append(result)
                    self.critical_failures.append(result)
        ...
        for var in ["SERIAL_PORT", "ADMIN_PORT"]:
            ...
            if port:
                if port in available_ports:
                    self.results.append(
                        ValidationResult(
                            True, f"{var} ({port}) is available", {"port": port}
                        )
                    )
                else:
                    self.results.append(
    ...
    def _print_results(self):
        ...
        self.logger.info("\n" + "=" * 60)
        self.logger.info(
            f"SUMMARY: {passed} passed, {failed} failed, {warnings} warnings"
        )
        self.logger.info("=" * 60)
