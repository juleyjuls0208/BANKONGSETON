...
# ============= ADMIN-ONLY FEATURES =============
# Card Registration and Lost Card Management (hardware-free data operations only)

# Helper functions for Arduino communication (no-ops when serial unavailable)
def send_display(line1, line2=""):
    """Send display command to Arduino (no-op if not connected)"""
...
def send_success(message):
    """Send success beep to Arduino (no-op if not connected)"""
    if not SERIAL_AVAILABLE:
        return
    arduino = card_reader_state.get('arduino')
    if arduino and arduino.is_open:
...
def send_error(message):
    """Send error beep to Arduino (no-op if not connected)"""
    if not SERIAL_AVAILABLE:
        return
    arduino = card_reader_state.get('arduino')
    if arduino and arduino.is_open:
...
def connect_serial():
    """Connect to Arduino"""
    if not SERIAL_AVAILABLE:
        return jsonify({'error': 'Serial not available in web mode'}), 503
    try:
        data = request.get_json()
        port = data.get('port')
...
@desktop_features
def start_register():
    """Start card registration process (requires Arduino hardware)"""
...
@desktop_features
def link_money_card():
    """Link money card to student (requires Arduino hardware)"""
    ard = card_reader_state.get('arduino')
    if not ard or not ard.is_open:
        return jsonify({'error': 'Arduino not connected'}), 400
