...
def get_ports():
    ...
    try:
        ...
        logger.warning(
            "event=serial_unavailable error=%s returning_empty_ports=True", e
        )
...
def process_sale():
    ...
    try:
        ...
        current_app.socketio.emit(
            "cashier_request_card", {"timeout": 5000, "total": total}
        )
...
def complete_sale():
    ...
    try:
        ...
        if not manual_student_id and card_uid and not UID_PATTERN.match(card_uid):
            return jsonify(
                {"error": "Card UID format is invalid -- please scan the card again"}
            ), 400
            ...
            try:
                ...
                _student = next(
                    (
                        r
                        for r in _users
                        if str(r.get("StudentID", "")) == manual_student_id
                    ),
                    None,
                )
                if not _student:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                last_error = e
                logger.warning(
                    f"complete_sale attempt {attempt}/{MAX_RETRIES} failed: {e}"
                )
