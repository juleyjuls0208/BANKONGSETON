...
class WriteQueue:
    ...
    def enqueue(
        ...
        self.logger.info(
            f"Queued {operation} for {sheet_name}. Queue size: {self.queue.qsize()}"
        )
            ...
            for _ in range(snapshot_size):
                try:
                    ...
                    if success:
                        ...
                        # Max 3 attempts
                        if item["attempts"] < 3:
                            self.queue.put(item)  # Re-queue for next pass
                        else:
                            self.failed_queue.append(item)
                            stats["failed"] += 1
                            self.logger.error(
                                f"Write failed after 3 attempts, moving to failed_queue: {item}"
                            )
...
class BatchRequest:
    ...
    def add_update(self, row: int, col: int, value: Any):
        ...
        self.updates.append(
            {"range": self._cell_address(row, col), "values": [[value]]}
        )
...
def enqueue_write(
    operation: str, sheet_name: str, data: Any, metadata: Optional[Dict] = None
):
