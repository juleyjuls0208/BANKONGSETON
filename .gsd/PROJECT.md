...
- Arduino UNO R4 WiFi firmware (`arduino/bankongseton_rfid/`) — dual-mode: APDU (HCE phone) + UID (physical RFID), WiFiS3, PN532 over SPI, HTTP POST to Flask backend, serial fallback; **M003/S01 fix: `httpPostCard(uid)` → `/api/arduino/card-read`, `httpPostNFC(token)` → `/api/nfc/tap`, dispatched by prefix in `deliver()`**; **M004/S01 fix: `inDataExchange` wrapped in 3-attempt retry loop (`APDU_MAX_RETRIES=3`, `APDU_RETRY_DELAY_MS=150`, lines 72-73 + 624-636); per-attempt `"APDU attempt N/3 ok=YES/NO rspLen=N"` diagnostic in Serial Monitor; pre-delay outside loop (RF field settle)**
- `backend/dashboard/cashier/cashier_routes.py` `complete_sale_nfc()` — D038-aligned Money Accounts lookup: direct `str().strip()` comparison (normalized_money_card removed per D032); `[NFC DEBUG]` prints retained for first hardware validation; `scripts/verify-m004.sh` 7/7 structural checks guard this path
...
- [x] M004: NFC Phone Payment Fix — contract complete (verify-m004.sh 7/7; firmware APDU retry loop + D038 backend alignment; hardware UAT pending human operator). See `.gsd/milestones/M004/M004-SUMMARY.md`.
